...
/**
 * ＠EnableGlobalMethodSecurity　開啟全局方法
 * 開啟方法註釋支持，我們設置 prePostEnabled = true 是為了後面能夠使用 hasRole() 這類表示式
 * 決定Spring Security的前注解是否可用 [@PreAuthorize,@PostAuthorize,..]
 * securedEnabled = true　決定是否Spring Security的保障注解 [@Secured] 是否可用
...
//                .key(SECRET_KEY)    // 此 SECRET 需要和生成 TokenBasedRememberMeService 的金鑰相同
        ...
        .and()
        .csrf().disable() // 關閉 CSRF，請求時才不需要另外帶 CSRF token
